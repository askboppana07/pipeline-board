// Salesforce connector — real SOQL via jsforce.
// Auth: username + password + security token (simple). For servers, prefer OAuth JWT bearer flow.
...
// Map Deployment__c.Status__c -> board state. Values confirmed with you.
...
// Build the Salesforce pipeline: environments (from Deployment__c) + blocked work items.
export async function fetchSalesforcePipeline() {
  ...
  const blockedStories = await c.query(
  ...
  const blockedDefects = await c.query(
    `SELECT Name, Validation_Status__c, Approval_Status__c FROM ${defect}
     WHERE Release__c = '${release.Id}' AND (Validation_Status__c='Failed' OR Approval_Status__c='Rejected')`);

  const blocked = [...blockedStories.records.map(r => ({ type: 'User Story', name: r.Name, reason: r.Validation_Status__c === 'Failed' ? 'Validation failed' : 'Approval rejected' })),
                   ...blockedDefects.records.map(r => ({ type: 'Defect', name: r.Name, reason: r.Validation_Status__c === 'Failed' ? 'Validation failed' : 'Approval rejected' }))];

  return {
    key: 'sf', tech: 'Salesforce', name: 'Salesforce', color: '#00A1E0',
    release: release.Name,
    envs: deployments.records.map(d => ({
      name: d.Environment__c, owner: d.Deployment_Owner__c || '—',
      branch: d.Branch__c, st: normalizeSfStatus(d.Status__c)
    })),
    blocked
  };
}
